...
const Header = () => {
  ...
  return (
    <header
      className={`navbar fixed top-0 left-0 right-0 z-50 border-b transition-all duration-300 transform-gpu
        ${
          isScrolled
            ? "bg-black/90 border-pink-500/30 py-3"
            : "bg-transparent border-transparent py-4"
        }
        ${isMobile ? "backdrop-blur-sm bg-black/80" : "backdrop-blur-md"}`}
    >
        ...
        <nav className="hidden md:flex items-center gap-6 lg:gap-8">
          {["HOME", "SKILLS", "PROJECTS", "ABOUT", "CONTACT"].map((item) => (
            <a
              ...
              className={`relative text-sm font-medium tracking-wider transition-all duration-300 group text-white hover:text-pink-200`}
              onClick={(e) => {
                e.preventDefault();
